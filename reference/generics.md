...
- 型パラメータは1つ以上をカンマ区切りで並べる。空リスト `<>` は書けない。型パラメータは **大文字で始まらなければならない** (MUST)。小文字始まりの識別子は effect-row パラメータ（[Effects §7](effects.md#7-effect-row-多相)）であり、関数の `<...>` でのみ宣言できる。
...
- effect row を row 変数で受ける多相は [effect-row 多相](effects.md#7-effect-row-多相)（row パラメータは小文字始まりの識別子として `<...>` 内に宣言する）。
