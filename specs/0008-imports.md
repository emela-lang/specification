...
実装は、外部 declaration を Platform manifest から読み込めます。manifest は
capability provider と backend binding manifest を兼ねます。
...
`params` と `return` は外部関数の型です。初期 manifest 形式では primitive type
`I32`, `Bool`, `Unit` を MUST support します。より複雑な named type や function type
を manifest にどう表すかは後続仕様で定義します。

`effectful` は呼び出しが effectful かどうかを表します。`capabilities` はその外部
関数を呼ぶために必要な capability set です。manifest 内で未知の type、未知の
...
`platform.io.print_i32!` の JavaScript lowering は、その外部関数の `bindings.js` に
...
`callee` は JavaScript expression として扱われ、code generation は
`callee(arg0, arg1, ...)` へ lower します。
...
runtime library 名の配列です。`--emit-asm` のように assembly だけを出力する mode では
...
implementation API であり、標準ライブラリ wrapper から呼ばれる外部実装境界です。
たとえば public API としての `std.io.print_i32!` は共通 wrapper とし、実装差分は
`platform.io.print_i32!` の manifest binding で吸収します。
