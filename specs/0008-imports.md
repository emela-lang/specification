...
package name が `std` の import は、通常の source package import として解決されます。

```emela
import std.io.write_stdout_utf8!
```

この import は、package `std` の source root にある `io.emel` の `write_stdout_utf8!` と、その
実装が呼び出す依存だけを現在の compilation unit へ導入します。`std.<module>.<item>`
は対応する `<module>.emel` source file を読み込みますが、同じ module 内の未使用
...
package `std` が解決済み package set にない場合、標準ライブラリ module が存在しない場合、
または指定された item を export しない場合、compiler は import を MUST reject します。
...
package `std` 内の `platform.*` import は stdlib internal import として扱われます。
