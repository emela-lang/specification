...
対象は、関数パラメータ、関数戻り値、ローカル束縛です。型注釈は型推論を置き換える
ものではなく、推論された型を制約し、関数境界や generic type の使用箇所を明示する
ために使います。

## Motivation

初期の Emela は関数パラメータ型を本体と呼び出し元から推論できます。しかし、次の
ケースでは明示的な型注釈が必要です。

- public API の関数境界を固定したい。
- 呼び出し元だけでは型が決まらない関数パラメータを扱いたい。
- `Result<I32, Error>` のような generic type の具体的な型引数を指定したい。
- ローカル束縛に期待型を与えて、型エラーを発生箇所に近づけたい。
...
パラメータ型注釈が存在する場合、そのパラメータの型は注釈された型でなければなり
ません。関数本体内の使用、および呼び出し時の実引数は、その型と一致しなければ
なりません。

パラメータ型注釈が存在しない場合、そのパラメータの型は従来通り推論されます。
ただし、関数の型を決定できない場合、実装はそのプログラムを MUST reject します。
...
束縛型注釈が存在する場合、右辺式の型は注釈された型と一致しなければなりません。
束縛型注釈が存在しない場合、束縛の型は右辺式から推論されます。
