...
```text
write_stdout_utf8!: (String) -> Result<Unit, PlatformError> !{IO, Platform} requires {Stdout}
```
...
`write_stdout_utf8!` が `Stdout` を要求する場合、`main!` も `Stdout` を要求すると推論されます。
...
`write_stdout_utf8!` が `Stdout` を要求する場合、`main!` も `Stdout` を要求します。ただし、
...
capability requirement を満たす外部実装 surface です。target triple は native ABI、
object format、linker などを選ぶための概念として残ります。
言い換えると、Backend profile は lowering と runtime/target 固有 binding の責務を持ち、
Platform はその profile が compiler へ公開する capability と外部関数 declaration です。

同じ backend に対して複数の runtime profile を選べます。たとえば `js-node` と
`js-bun` は同じ JavaScript lowering を共有しつつ、それぞれの runtime binding を提供
できます。native backend では `native-aarch64-apple-darwin` と
`native-x86_64-unknown-linux-gnu` が別 profile になります。

Platform は backend が compiler へ提供する API surface として扱います。backend は
lowering の責務を持ち、その descriptor は少なくとも profile 名、backend kind、ABI
version、起動 command または built-in backend kind、runtime/target、提供 capability
set、外部関数 declaration を含みます。`js + node`、`js + bun`、
`native + aarch64-apple-darwin` はそれぞれ別 backend profile として表します。
...
```json
{
  ...
  "runtime": "node",
  "capabilities": ["Stdout", "Stdin", "Clock"],
  "externs": [
    {
      "path": ["platform", "io"],
...
外部 backend は compiler process の外側で実行されます。compiler は typecheck 済み IR、
target、compilation mode、解決済み external import を versioned JSON request として
backend process の stdin へ渡します。backend は artifact または diagnostics を JSON
response として stdout に返します。ABI version が一致しない backend は MUST reject
されます。
...
Backend は capability を提供するだけでなく、自身の lowering に必要な binding も該当
外部関数ごとに提供しなければ code generation できません。

Platform primitive の失敗は panic/trap ではなく `Result` 値として返します。初期の
I/O primitive は `Result<_, PlatformError>` を返し、`PlatformError` は platform
runtime が返せる失敗分類を表す enum です。
...
または platform runtime 関数へ lower できます。初期 Native backend binding は
