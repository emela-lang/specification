...
Emela compilation unit はトップレベル関数定義の集合です。実装は、少なくとも次の
2 つの mode を区別できます。

- executable mode
- library mode

executable mode の compilation unit には、`main` または `main!` という名前の
トップレベル関数がちょうど 1 つ MUST 存在します。
...
`main` / `main!` は 0 個の引数を取ります。戻り値型は `Unit` または `I32` です。
`Unit` を返す場合、実行可能ファイルは成功として終了します。`I32` を返す場合、
その値をプロセス終了コードとして扱います。

library mode の compilation unit は executable entry point を要求しません。library
mode では、`main` / `main!` が存在しなくても型検査、外部 import 解決、effect/capability
検査を実行できます。library mode の出力は、別の compilation unit から参照される
関数群、metadata、object file、または backend-specific module であり、単体で実行
可能であることを要求しません。

library mode では runtime boundary が選択されません。そのため、entry point に対する
process exit code lowering や platform capability の最終提供チェックは executable
mode または link/package composition の段階で行います。ただし、各関数内の型、
effect、明示された `#[requires(...)]` との整合性は library mode でも通常通り
MUST 検査します。
...
関数定義は戻り値型注釈を持つことができます。戻り値型注釈は `-> Type` の形で、
引数リストの直後、関数本体ブロックの直前に書きます。
...
戻り値型注釈が存在する場合、関数本体ブロックの型は注釈された型と一致しなければ
なりません。戻り値型注釈が存在しない場合、戻り値型は関数本体から推論されます。
...
`match` は式です。scrutinee 式を評価し、その値に一致する最初の arm の式を評価します。
...
このドラフトでは、pattern は整数リテラル、`true`、`false`、`()`、wildcard `_` です。
wildcard `_` は任意の値に一致します。

すべての arm の pattern は scrutinee と同じ型を MUST 持ちます。wildcard `_` は
scrutinee の型を持つものとして扱います。

すべての arm の式は同じ型を MUST 持ちます。

`match` は exhaustive でなければなりません。このドラフトでは、次の場合に exhaustive
とみなします。

- wildcard `_` arm を持つ。
- `Bool` に対する `true` と `false` の両方の arm を持つ。
- `Unit` に対する `()` arm を持つ。
...
関数の戻り値型注釈は任意です。注釈が存在する場合、型検査器は関数本体の型が
注釈された型と一致することを MUST 検査します。

このドラフトでは仮引数とローカル束縛の型注釈を要求しません。後続仕様で、それらの
型注釈の構文と正確な推論アルゴリズムを追加できます。
...
Native ターゲットでは、`main` が `I32` を返す場合、その値をプロセス終了コードへ
lower します。`main` が `Unit` を返す場合、成功終了へ lower します。
...
- 仮引数とローカル束縛は最初のバージョンから明示的な型注釈を持つべきか。
- `main` の `Bool` 戻り値を終了コードへ対応づけるべきか。
