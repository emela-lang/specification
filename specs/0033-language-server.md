...
コンパイラが発するすべての診断（字句・構文・import・型・trait 実装漏れ・effect・throws・entrypoint），
文脈に応じた補完（import 候補，`match` / `catch` の enum バリアント，`uses` 行の effect，予約語ほか），
型情報のホバー，および quickfix コードアクション（`match` の不足アーム挿入・`impl` のスタブ生成）を
提供する．
...
- ホバー（`textDocument/hover`）は，検査時に記録した **span→型 の型インデックス**を引き，
  パラメータ・`let` 束縛（注釈なしの推論型を含む）・変数参照・任意の式の型と，関数名の
  シグネチャを表示する．
- コードアクション（`textDocument/codeAction`）は 2 種の quickfix を返す: 非網羅 `match` への
  不足アームの一括挿入と，`impl` の未実装メソッドのスタブ一括生成．いずれも診断の payload に
  依存せず，要求範囲からスナップショットに基づき再計算する．
...
| 要求 | `textDocument/hover` | 型インデックスを引き，カーソル位置の型情報を返す（該当なしは `null`） |
| 要求 | `textDocument/codeAction` | 範囲から再計算した quickfix の配列を返す（該当なしは空配列） |
...
- 変換規則: 各エラーの `Diagnostic` から LSP `Diagnostic` を作る．
  ...
  - code: 機械可読なコードを持つ診断はそれを載せる．現在は `non-exhaustive-match`（非網羅
    `match`）と `incomplete-impl`（`impl` の実装漏れ）——対応するコードアクションの手がかりと
    してクライアントが利用できる．lint 診断は規則 id（0035 L4）を用いる．code を持つ診断は
    CLI 出力でもタイトルの後に `[code]` が付く．
...
型検査は **span→型 の型インデックス**を記録する（LSP 経由の検査のみ．`check` / `build` 等の
通常コンパイルは記録せず，コストを払わない）．すべての式は検査の単一合流点で，パラメータ・
`let`・パターン束縛は名前の導入地点で，それぞれ記録される．関数本体の検査は最初のエラーで
打ち切られるが，そこまでに検査済みの記録は残るため，エラーのあるファイルでもホバーは動き続ける．

`textDocument/hover` はカーソル位置を次の順で解決し，最初に該当したものを返す:

1. カーソル位置の識別子と span が完全一致するエントリ．束縛（パラメータ・`let`・パターン束縛）は
   `name: Type`，式は型のみを表示する．注釈のない `let` は推論された型が，ジェネリック関数の
   呼び出しは型引数を解決済みの具象型が出る．関数型の参照（呼び出しの callee 等）は
   `fn name(P) -> R throws E uses { .. }` のシグネチャ形式で表示する．
2. スコープスナップショット中の関数・trait メソッド名（定義行など，インデックスにない名前）:
   整形済みシグネチャ．
3. enum 名: `enum Name`．
4. カーソルを含む**最小 span** のエントリ — 演算子や区切り記号の上では囲む式の型が出る．
5. 該当なし: 結果 `null`（エラーではない）．

表示は markdown（`` ```emela `` フェンスの 1 行）で，`range` に対象の span を返す．
`Some(v)` パターンの payload 束縛 `v` は導入地点の span を持たないため対象外（アーム本体内の
使用箇所は対象）．

### コードアクション

`textDocument/codeAction` は，クライアントが echo する `context.diagnostics` に依存せず，
要求された範囲から**スナップショットに基づき再計算**する．サーバは変更のたびに同期的に再検査
するためスナップショットは常に最新であり，診断を選択していなくても該当位置にカーソルがあれば
アクションは提示される．返す kind は `quickfix` のみ．編集は対象文書 1 ファイルへの単一
`TextEdit`（`WorkspaceEdit.changes`）である．

- **不足アームの挿入（非網羅 `match`）**: 範囲を含む最内の `match` について，網羅性の規則
  （0005 と同一: ガード付きアームは網羅に数えない，wildcard / 束縛パターンは catch-all）で
  不足バリアントを再計算し，最終アームの直後（アームがなければ開き `{` の直後）に宣言順で
  挿入する．挿入形は `Enum::Variant(_) -> panic("TODO: handle Variant")` — payload は `_` で
  無視し，`panic` は `Never` に型付くため，挿入直後からコンパイルが通る．scrutinee の enum は
  型インデックスから解決する（網羅性エラーの前に scrutinee は検査済みのため，壊れた `match`
  でも引ける）．
- **impl スタブの生成（`Incomplete impl`）**: 範囲を含む `impl` ブロックについて，trait 宣言との
  差分から default のない未実装メソッドを**全件**再導出し（コンパイラの報告は最初の 1 件のみ），
  最後の既存メソッドの直後（なければ開き `{` の直後）にスタブを挿入する．シグネチャは `Self` を
  対象型に置換して表層構文で描画し，`throws` / `uses` 行は trait の宣言と厳密に一致させる
  （0020 の impl シグネチャ検査は完全一致を要求する）．本体は
  `panic("TODO: implement name")` である．
...
- go-to-definition / rename 等の追加機能．
- ホバーの残課題: パターン payload 束縛（`Some(v)` の `v`）の導入地点，コメント・文字列内の語の
  除外（トークン分類ゲート）．
- コードアクションの残課題: `catch` アームの補充（`try` 経由の非網羅も同じ診断を出すが，
  エラー型は throws チャネル上にあり型インデックスに載らない）．
